...
function IsLastGrade(grade)
{
	if (!grades || !grades.length)
		return false;
	const LastGrade = grades[18];
	return grade === LastGrade;
}

function UpdatePlayerProgress(score)
{
	const grade = GetGrade(score);
	if (!grade)
		return;
	if (IsLastGrade(grade))
		GoToEndingPage();
}
...
function createFallingMushroom()
{
	...
	else if (isCash)
	{
		...
		mushroom.addEventListener('click', (e) => {
			e.stopPropagation();
			const cashAmount = Math.floor(Math.random() * 5) + 1;
			cash += cashAmount;
			localStorage.setItem('mushroomCash', cash.toString());
			updateCashDisplay();
			mushroom.remove();
...
function updateStats()
{
	...
	UpdatePlayerProgress(totalMushroomsEver);
}
...
function createFireworks()
{
	...
	const explosionCount = Math.min(3 + Math.floor(gradeLevel / 3), 10);
	const particlesPerExplosion = Math.min(12 + gradeLevel * 2, 40);
	const particleSize = Math.min(8 + gradeLevel * 0.5, 16);
	const velocityMultiplier = 1 + (gradeLevel * 0.1);
